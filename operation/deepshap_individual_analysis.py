...
def compute_shap_and_prognostic_factors(repeat, fold, args, num_features):
    """计算单个实验的DeepSHAP值并识别预后因子
    
    分析流程：
    1. DeepSHAP分析（Top 100特征）：
       - Driver因子: SHAP均值显著 > 0 (对风险的正向贡献)
       - Protector因子: SHAP均值显著 < 0 (对风险的负向贡献)
       - 统计检验: t检验测试SHAP值是否显著偏离0
    2. Cox回归分析（Top 100特征）：
       - Risk因子: HR > 1 且显著
    ...
    # 🔥 关键修改：使用DeepExplainer替代GradientExplainer
    print(f"计算DeepSHAP值...")
    explainer = shap.DeepExplainer(RiskWrapper(model), background_t)
    # 禁用additivity检查以避免数值误差导致的断言失败
    shap_values = explainer.shap_values(X_val_t, check_additivity=False)
    
    # 处理shap_values的形状
    if isinstance(shap_values, list):
        shap_values = shap_values[0]  # 如果返回列表，取第一个
    if isinstance(shap_values, torch.Tensor):
        shap_values = shap_values.cpu().numpy()  # 转换为numpy数组
    ...
    # 计算风险分数
    with torch.no_grad():
        logits = model(x_omic=X_val_t)
        hazards = torch.sigmoid(logits)
        survival = torch.cumprod(1 - hazards, dim=1)
        risk_scores = -torch.sum(survival, dim=1).cpu().numpy()
    ...
    importance_file = os.path.join(importance_dir, f"repeat{repeat}_fold{fold}_deepshap_feature_importance_ranking.csv")
    ...
    for i in top_100_indices:
        shap_feature = shap_values[:, i]  # 该特征在所有样本中的SHAP值
        
        # 核心逻辑：测试SHAP值是否显著偏离0
        try:
            stat, pval = ttest_1samp(shap_feature, 0)
        except:
            pval = 1.0
            stat = 0
        
        mean_shap = np.mean(shap_feature)
        std_shap = np.std(shap_feature)
        
        # 计算一致性指标
        consistency_ratio = np.abs(np.sign(shap_feature).sum()) / len(shap_feature)
        positive_ratio = (shap_feature > 0).mean()
        
        prognostic_factors.append({
            'gene': feature_names[i],
            'importance': importance[i],
            'mean_shap': mean_shap,
            'abs_mean_shap': abs(mean_shap),  # 绝对平均值
            'std_shap': std_shap,
            'ttest_stat': stat,
            'p_value': pval,
    ...
    if p_values:
        ...
        for i, (factor, adj_p, sig) in enumerate(zip(prognostic_factors, p_adj, reject)):
            ...
            if sig:  # FDR显著偏离0
                significant_count += 1
                # 正确且简单的逻辑：直接基于SHAP均值的符号
                if factor['mean_shap'] > 0:
                    factor['type'] = 'driver'      # SHAP > 0: 增加风险
                    driver_count += 1
                elif factor['mean_shap'] < 0:
                    factor['type'] = 'protector'   # SHAP < 0: 降低风险
                    protector_count += 1
    ...
    result = {
        ...
        'prognostic_factors': prognostic_factors,  # 每个基因的详细SHAP统计信息在这里
        ...
        'analysis_method': 'DeepSHAP_plus_Cox_Analysis'  # 标记使用DeepSHAP+Cox分析
    }
    ...
    print(f"统计显著因子数量: {significant_count} (SHAP值显著偏离0)")
...
def analyze_cancer_type(args, num_features, num_repeats=10, num_folds=5):
    ...
    for repeat in range(num_repeats):
        for fold in range(num_folds):
            result = compute_shap_and_prognostic_factors(repeat, fold, args, num_features)
    ...
    # 保存每个实验的Cox分析详细结果（包含每个基因的SHAP统计信息）
    all_cox_details = []
    for r in all_results:
        if 'cox_results' in r and r['cox_results'] and 'prognostic_factors' in r:
            # 创建基因名到SHAP统计信息的映射
            shap_stats_map = {pf['gene']: pf for pf in r['prognostic_factors']}
            ...
            for cox_result in r['cox_results']:
                ...
                # 添加该基因的SHAP统计信息
                if gene_name in shap_stats_map:
                    shap_stats = shap_stats_map[gene_name]
                    cox_detail.update({
                        'mean_shap': shap_stats['mean_shap'],
                        'abs_mean_shap': shap_stats['abs_mean_shap'],
                        'shap_importance': shap_stats['importance'],
                        'shap_p_value': shap_stats['p_value'],
                        'shap_p_adj': shap_stats.get('p_adj', None),
                        'shap_type': shap_stats.get('type', 'not_significant')
                    })
...
def create_correlation_plots(cancer, df, results_dir):
    """创建相关性图（包含DeepSHAP和Cox分析）- 🔧 添加拟合直线和统计信息"""
    ...
    plot_with_fit(axes[0,0], df['cindex'], df['deepshap_prognostic_factors_raw'], 
                  ...
                  f'{cancer}: C-index vs DeepSHAP Prognostic Factors', 'purple')
    ...
    plot_with_fit(axes[0,1], df['cindex'], df['deepshap_driver_factors_raw'], 
                  ...
                  f'{cancer}: C-index vs DeepSHAP Driver Factors', 'red')
    ...
    plot_with_fit(axes[0,2], df['cindex'], df['deepshap_protector_factors_raw'], 
                  ...
                  f'{cancer}: C-index vs DeepSHAP Protector Factors', 'blue')
...
def main(args):
    """主函数 - DeepSHAP分析 + Cox回归分析"""
    print("🔥 综合分析框架: DeepSHAP + Cox回归")
    print("=== DeepSHAP分析部分 ===")
    print("   ✅ Driver因子: SHAP均值显著 > 0 (增加风险)")
    print("   ✅ Protector因子: SHAP均值显著 < 0 (降低风险)")
    print("   ✅ 统计检验: t检验测试SHAP值是否显著偏离0")
    print("=== Cox回归分析部分 ===")
    ...
    if all_cancer_summaries:
        ...
        summary_df.to_csv(os.path.join(args.deepshap_dir, "all_cancers_summary.csv"), index=False)
        
        print(f"\n{'='*60}")
        print("所有癌症类型汇总 (DeepSHAP + Cox分析):")
        print(f"{'='*60}")
        
        # DeepSHAP分析汇总 - 主要结果（原始p值）
        print("=== DeepSHAP分析汇总（主要结果：使用原始p值预后因子数量）===")
        deepshap_cols_raw = ['cancer', 'cindex_mean', 'deepshap_prognostic_factors_raw_mean', 
                        'corr_cindex_deepshap_prognostic_raw_pearson', 'corr_cindex_deepshap_prognostic_raw_pearson_p']
        print(summary_df[deepshap_cols_raw].to_string(index=False))
        ...
        # 对比：FDR校正后的结果
        print("\n=== 对比：FDR校正后的相关性 ===")
        compare_cols = ['cancer', 'deepshap_prognostic_factors_fdr_mean', 'cox_prognostic_factors_fdr_mean',
                       'corr_cindex_deepshap_prognostic_fdr_pearson', 'corr_cindex_cox_prognostic_fdr_pearson']
        ...
        print(f"   DeepSHAP预后因子: 基于SHAP值是否显著偏离0来判定")
        print(f"   - Driver(增加风险): SHAP > 0")
        print(f"   - Protector(降低风险): SHAP < 0")
        ...
        print(f"   📁 特征重要性文件: {args.deepshap_dir}/[癌症类型]/feature_importance/")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="单独分析每个实验的DeepSHAP值和预后因子")
