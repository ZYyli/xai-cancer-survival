...
def compute_shap_and_prognostic_factors(repeat, fold, args, num_features):
    ...
    importance_file = os.path.join(importance_dir, f"repeat{repeat}_fold{fold}_shap_feature_importance_ranking.csv")
    ...
    result = {
        ...
        'analysis_method': 'SHAP_plus_Cox_Analysis'  # 标记使用SHAP+Cox分析
    }
...
def create_correlation_plots(cancer, df, results_dir):
    """创建相关性图（包含SHAP和Cox分析）- 🔧 添加拟合直线和统计信息"""
    ...
    plot_with_fit(axes[0,2], df['cindex'], df['shap_protector_factors_raw'], 
                  ...
                  f'{cancer}: C-index vs SHAP Protector Factors', 'blue')
...
def main(args):
    ...
    if all_cancer_summaries:
        ...
        summary_df.to_csv(os.path.join(args.shap_dir, "all_cancers_summary.csv"), index=False)
        
        print(f"\n{'='*60}")
        print("所有癌症类型汇总 (SHAP + Cox分析):")
        print(f"{'='*60}")
        
        # SHAP分析汇总 - 主要结果（原始p值）
        print("=== SHAP分析汇总（主要结果：使用原始p值预后因子数量）===")
        shap_cols_raw = ['cancer', 'cindex_mean', 'shap_prognostic_factors_raw_mean', 
                        'corr_cindex_shap_prognostic_raw_pearson', 'corr_cindex_shap_prognostic_raw_pearson_p']
        print(summary_df[shap_cols_raw].to_string(index=False))
        ...
        # 对比：FDR校正后的结果
        print("\n=== 对比：FDR校正后的相关性 ===")
        compare_cols = ['cancer', 'shap_prognostic_factors_fdr_mean', 'cox_prognostic_factors_fdr_mean',
                       'corr_cindex_shap_prognostic_fdr_pearson', 'corr_cindex_cox_prognostic_fdr_pearson']
        ...
        print(f"   SHAP预后因子: 基于SHAP值是否显著偏离0来判定")
        ...
        print(f"   📁 特征重要性文件: {args.shap_dir}/[癌症类型]/feature_importance/")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="单独分析每个实验的SHAP值和预后因子")
