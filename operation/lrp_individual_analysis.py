...
def compute_lrp(model, X, device, epsilon=1e-6):
    model.eval()
    lrp = LRP(model, epsilon=epsilon)
    X_tensor = torch.tensor(X, dtype=torch.float32).to(device)
    relevance = lrp.propagate(X_tensor)
    return relevance.detach().cpu().numpy()

def compute_lrp_and_prognostic_factors(repeat, fold, args, num_features):
    """计算单个实验的LRP值并识别预后因子"""
    ...
    print(f"计算LRP值...")
    # 计算LRP relevance
    relevance = compute_lrp(model, X_val_np, device)
    ...
    # 计算特征重要性（用于筛选）
    importance = np.abs(relevance).mean(axis=0)
    ...
    importance_file = os.path.join(importance_dir, f"repeat{repeat}_fold{fold}_lrp_feature_importance_ranking.csv")
    ...
    for i in top_100_indices:
        lrp_feature = relevance[:, i]  # 该特征在所有样本中的LRP值
    ...
    result = {
        ...
        'analysis_method': 'LRP_plus_Cox_Analysis'
    }
...
def analyze_cancer_type(args, num_features, num_repeats=10, num_folds=5):
    ...
    for repeat in range(num_repeats):
        for fold in range(num_folds):
            result = compute_lrp_and_prognostic_factors(repeat, fold, args, num_features)
    ...
    # 保存每个实验的Cox分析详细结果（包含每个基因的LRP统计信息）
    all_cox_details = []
    for r in all_results:
...
def create_correlation_plots(cancer, df, results_dir):
    """创建相关性图（包含LRP和Cox分析）- 🔧 添加拟合直线和统计信息"""
...
def main(args):
    """主函数 - LRP分析 + Cox回归分析"""
    print("🔥 综合分析框架: LRP + Cox回归")
    print("=== LRP分析部分 ===")
    print("   ✅ Driver因子: LRP均值显著 > 0 (增加风险)")
    print("   ✅ Protector因子: LRP均值显著 < 0 (降低风险)")
    print("   ✅ 统计检验: t检验测试LRP值是否显著偏离0")
    print("=== Cox回归分析部分 ===")
    ...
    if all_cancer_summaries:
        ...
        summary_df.to_csv(os.path.join(args.lrp_dir, "all_cancers_summary.csv"), index=False)
        
        print(f"\n{'='*60}")
        print("所有癌症类型汇总 (LRP + Cox分析):")
        print(f"{'='*60}")
        ...
        print(f"   📁 特征重要性文件: {args.lrp_dir}/[癌症类型]/feature_importance/")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="单独分析每个实验的LRP值和预后因子")
