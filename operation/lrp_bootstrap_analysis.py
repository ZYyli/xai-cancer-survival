...
def compute_lrp_and_prognostic_factors(bootstrap_seed, args, num_features, feature_importance_only=False):
    """计算单个bootstrap模型的LRP值并识别预后因子
    ...
    X_oob_t = torch.tensor(X_oob, dtype=torch.float32).to(device)
    ...
    # 创建输出目录
    # 输出路径: args.lrp_dir/[癌症]/lrp_feature_importance/
    importance_dir = os.path.join(args.lrp_dir, args.cancer, 'lrp_feature_importance')
    if not os.path.exists(importance_dir):
        os.makedirs(importance_dir, exist_ok=True)
        print(f"📁 创建目录: {importance_dir}")
    
    # 文件名: seed{X}_lrp_ranking.csv
    importance_file = os.path.join(importance_dir, f"seed{bootstrap_seed}_lrp_ranking.csv")
    feature_importance_df.to_csv(importance_file, index=False)
    print(f"✅ LRP特征重要性排序已保存: {importance_file}")
    ...
    # 正确逻辑：直接基于LRP值是否显著偏离0
    print(f"使用正确逻辑：基于LRP值的直接生物学意义")
    print(f"Driver: LRP均值显著 > 0 (增加风险)")
    print(f"Protector: LRP均值显著 < 0 (降低风险)")
    
    for i in top_100_indices:
        lrp_feature = relevance_np[:, i]  # 该特征在所有OOB样本中的LRP值
        
        # 核心逻辑：测试LRP值是否显著偏离0
        try:
            stat, pval = ttest_1samp(lrp_feature, 0)
        except:
            pval = 1.0
            stat = 0
        
        mean_lrp = np.mean(lrp_feature)
        std_lrp = np.std(lrp_feature)
        
        # 计算一致性指标
        consistency_ratio = np.abs(np.sign(lrp_feature).sum()) / len(lrp_feature)
        positive_ratio = (lrp_feature > 0).mean()
        
        prognostic_factors.append({
            'gene': feature_names[i],
            'importance': importance[i],
            'mean_lrp': mean_lrp,
            'abs_mean_lrp': abs(mean_lrp),  # 绝对平均值
            'std_lrp': std_lrp,
            'ttest_stat': stat,
            'p_value': pval,
    ...
    if p_values:
        ...
        for i, (factor, adj_p, sig) in enumerate(zip(prognostic_factors, p_adj, reject)):
            ...
            if sig:  # FDR显著偏离0
                significant_count += 1
                # 正确且简单的逻辑：直接基于LRP均值的符号
                if factor['mean_lrp'] > 0:
                    factor['type'] = 'driver'      # LRP > 0: 增加风险
                    driver_count += 1
                elif factor['mean_lrp'] < 0:
                    factor['type'] = 'protector'   # LRP < 0: 降低风险
                    protector_count += 1
                else:
    ...
    result = {
        ...
        # LRP分析结果 - FDR校正后
        'lrp_prognostic_factors_fdr': prognostic_count_fdr,
        'lrp_driver_factors_fdr': driver_count,
        'lrp_protector_factors_fdr': protector_count,
        
        # LRP分析结果 - 原始p值
        'lrp_prognostic_factors_raw': prognostic_count_raw,
        'lrp_driver_factors_raw': driver_count_raw,
        'lrp_protector_factors_raw': protector_count_raw,
        
        'prognostic_factors': prognostic_factors,  # 每个基因的详细LRP统计信息在这里
        ...
        'analysis_method': 'Bootstrap_LRP_plus_Cox_Analysis'
    }
    
    print(f"C-index: {cindex:.4f}")
    print(f"=== LRP分析结果 ===")
    print(f"统计显著因子数量: {significant_count} (LRP值显著偏离0)")
...
def analyze_bootstrap_cancer(args, num_features, num_bootstraps=100, feature_importance_only=False):
    ...
    for bootstrap_seed in range(1, num_bootstraps + 1):
        result = compute_lrp_and_prognostic_factors(bootstrap_seed, args, num_features, feature_importance_only)
    ...
    # LRP分析统计数据 - FDR校正后
    lrp_prognostic_counts_fdr = [r['lrp_prognostic_factors_fdr'] for r in all_results]
    lrp_driver_counts_fdr = [r['lrp_driver_factors_fdr'] for r in all_results]
    lrp_protector_counts_fdr = [r['lrp_protector_factors_fdr'] for r in all_results]
    
    # LRP分析统计数据 - 原始p值
    lrp_prognostic_counts_raw = [r['lrp_prognostic_factors_raw'] for r in all_results]
    lrp_driver_counts_raw = [r['lrp_driver_factors_raw'] for r in all_results]
    lrp_protector_counts_raw = [r['lrp_protector_factors_raw'] for r in all_results]
    ...
    stats_summary = {
        ...
        # LRP分析统计 - FDR校正后
        'lrp_prognostic_factors_fdr_mean': np.mean(lrp_prognostic_counts_fdr),
        'lrp_prognostic_factors_fdr_std': np.std(lrp_prognostic_counts_fdr),
        'lrp_driver_factors_fdr_mean': np.mean(lrp_driver_counts_fdr),
        'lrp_driver_factors_fdr_std': np.std(lrp_driver_counts_fdr),
        'lrp_protector_factors_fdr_mean': np.mean(lrp_protector_counts_fdr),
        'lrp_protector_factors_fdr_std': np.std(lrp_protector_counts_fdr),
        
        # LRP分析统计 - 原始p值
        'lrp_prognostic_factors_raw_mean': np.mean(lrp_prognostic_counts_raw),
        'lrp_prognostic_factors_raw_std': np.std(lrp_prognostic_counts_raw),
        'lrp_driver_factors_raw_mean': np.mean(lrp_driver_counts_raw),
        'lrp_driver_factors_raw_std': np.std(lrp_driver_counts_raw),
        'lrp_protector_factors_raw_mean': np.mean(lrp_protector_counts_raw),
        'lrp_protector_factors_raw_std': np.std(lrp_protector_counts_raw),
    ...
    # 相关性分析 - 主要使用原始p值（校正前）的预后因子数量
    if len(cindices) > 2:
        # C-index与LRP预后因子总数的相关性 (原始p值 - 主要分析)
        corr_lrp_prog_raw, p_lrp_prog_raw = pearsonr(cindices, lrp_prognostic_counts_raw)
        spearman_lrp_prog_raw, sp_lrp_prog_raw = spearmanr(cindices, lrp_prognostic_counts_raw)
        
        # C-index与driver因子数量的相关性 (原始p值)
        corr_driver_raw, p_driver_raw = pearsonr(cindices, lrp_driver_counts_raw)
        spearman_driver_raw, sp_driver_raw = spearmanr(cindices, lrp_driver_counts_raw)
        
        # C-index与protector因子数量的相关性 (原始p值)
        corr_protector_raw, p_protector_raw = pearsonr(cindices, lrp_protector_counts_raw)
        spearman_protector_raw, sp_protector_raw = spearmanr(cindices, lrp_protector_counts_raw)
        ...
        # 附加：FDR校正后的相关性（用于对比）
        corr_lrp_prog_fdr, p_lrp_prog_fdr = pearsonr(cindices, lrp_prognostic_counts_fdr)
        corr_cox_prog_fdr, p_cox_prog_fdr = pearsonr(cindices, cox_prognostic_counts_fdr)
        
        stats_summary.update({
            # LRP相关性 - 原始p值（主要分析）
            'corr_cindex_lrp_prognostic_raw_pearson': corr_lrp_prog_raw,
            'corr_cindex_lrp_prognostic_raw_pearson_p': p_lrp_prog_raw,
            'corr_cindex_lrp_prognostic_raw_spearman': spearman_lrp_prog_raw,
            'corr_cindex_lrp_prognostic_raw_spearman_p': sp_lrp_prog_raw,
            ...
            # FDR校正后的相关性（辅助分析）
            'corr_cindex_lrp_prognostic_fdr_pearson': corr_lrp_prog_fdr,
            'corr_cindex_lrp_prognostic_fdr_pearson_p': p_lrp_prog_fdr,
            'corr_cindex_cox_prognostic_fdr_pearson': corr_cox_prog_fdr,
            'corr_cindex_cox_prognostic_fdr_pearson_p': p_cox_prog_fdr
        })
        
        print(f"=== LRP相关性分析（使用原始p值预后因子数量）===")
        print(f"C-index vs LRP预后因子总数 - Pearson: r={corr_lrp_prog_raw:.3f}, p={p_lrp_prog_raw:.3f}")
        ...
        print(f"对比FDR校正后 - LRP: r={corr_lrp_prog_fdr:.3f}, Cox: r={corr_cox_prog_fdr:.3f}")
    
    # 保存详细结果
    results_dir = os.path.join(args.lrp_dir, args.cancer)
    ...
    detailed_results_df = pd.DataFrame([{
        ...
        # LRP分析结果 - FDR校正后
        'lrp_prognostic_factors_fdr': r['lrp_prognostic_factors_fdr'],
        'lrp_driver_factors_fdr': r['lrp_driver_factors_fdr'],
        'lrp_protector_factors_fdr': r['lrp_protector_factors_fdr'],
        
        # LRP分析结果 - 原始p值
        'lrp_prognostic_factors_raw': r['lrp_prognostic_factors_raw'],
        'lrp_driver_factors_raw': r['lrp_driver_factors_raw'],
        'lrp_protector_factors_raw': r['lrp_protector_factors_raw'],
    ...
    # 保存每个bootstrap的Cox分析详细结果（包含每个基因的LRP统计信息）
    all_cox_details = []
    for r in all_results:
        if 'cox_results' in r and r['cox_results'] and 'prognostic_factors' in r:
            # 创建基因名到LRP统计信息的映射
            lrp_stats_map = {pf['gene']: pf for pf in r['prognostic_factors']}
            ...
            for cox_result in r['cox_results']:
                ...
                # 添加该基因的LRP统计信息
                if gene_name in lrp_stats_map:
                    lrp_stats = lrp_stats_map[gene_name]
                    cox_detail.update({
                        'mean_lrp': lrp_stats['mean_lrp'],
                        'abs_mean_lrp': lrp_stats['abs_mean_lrp'],
                        'lrp_importance': lrp_stats['importance'],
                        'lrp_p_value': lrp_stats['p_value'],
                        'lrp_p_adj': lrp_stats.get('p_adj', None),
                        'lrp_type': lrp_stats.get('type', 'not_significant')
                    })
...
def create_bootstrap_correlation_plots(cancer, df, results_dir):
    """创建bootstrap相关性图（包含LRP和Cox分析）- 🔧 添加拟合直线和统计信息"""
    ...
    # === 第一行：LRP分析结果（使用原始p值预后因子数量）===
    # C-index vs LRP预后因子总数
    plot_with_fit(axes[0,0], df['cindex'], df['lrp_prognostic_factors_raw'], 
                  'C-index', 'LRP Prognostic Factors (Raw p)', 
                  f'{cancer}: C-index vs LRP Prognostic Factors (Bootstrap)', 'purple')
    
    # C-index vs Driver因子数量
    plot_with_fit(axes[0,1], df['cindex'], df['lrp_driver_factors_raw'], 
                  'C-index', 'LRP Driver Factors (Raw p)', 
                  f'{cancer}: C-index vs LRP Driver Factors (Bootstrap)', 'red')
    
    # C-index vs Protector因子数量
    plot_with_fit(axes[0,2], df['cindex'], df['lrp_protector_factors_raw'], 
                  'C-index', 'LRP Protector Factors (Raw p)', 
                  f'{cancer}: C-index vs LRP Protector Factors (Bootstrap)', 'blue')
...
def main(args):
    ...
    os.makedirs(args.lrp_dir, exist_ok=True)
    
    all_cancer_summaries = []
    
    # 确定bootstrap数量
    num_bootstraps = getattr(args, 'num_bootstrap', 100)
    ...
    for cancer in cancer_list:
        ...
        num_features = 2000
        print(f"\n{'='*60}")
        print(f"正在处理癌症类型: {cancer} (Bootstrap LRP)")
    ...
    if all_cancer_summaries:
        ...
        summary_df.to_csv(os.path.join(args.lrp_dir, "all_cancers_bootstrap_summary.csv"), index=False)
        print(f"\n✅ 所有癌症汇总结果已保存到: {os.path.join(args.lrp_dir, 'all_cancers_bootstrap_summary.csv')}")
        
        # 打印总体统计
        if not args.feature_importance_only:
            print(f"\n{'='*60}")
            print("所有癌症类型Bootstrap汇总 (LRP + Cox分析):")
            print(f"{'='*60}")
            
            # LRP分析汇总 - 主要结果（原始p值）
            print("=== LRP分析汇总（主要结果：使用原始p值预后因子数量）===")
            lrp_cols_raw = ['cancer', 'cindex_mean', 'lrp_prognostic_factors_raw_mean', 
                           'corr_cindex_lrp_prognostic_raw_pearson', 'corr_cindex_lrp_prognostic_raw_pearson_p']
            print(summary_df[lrp_cols_raw].to_string(index=False))
            ...
            # 对比：FDR校正后的结果
            print("\n=== 对比：FDR校正后的相关性 ===")
            compare_cols = ['cancer', 'lrp_prognostic_factors_fdr_mean', 'cox_prognostic_factors_fdr_mean',
                           'corr_cindex_lrp_prognostic_fdr_pearson', 'corr_cindex_cox_prognostic_fdr_pearson']
            print(summary_df[compare_cols].to_string(index=False))
            
            print(f"\n💡 说明:")
            print(f"   🎯 主要分析：使用原始p值（校正前）的预后因子数量计算相关性")
            print(f"   📊 可视化：散点图包含拟合直线、相关系数和显著性检验")
            print(f"   🔄 Bootstrap: 对100个重抽样模型进行LRP分析，提供更稳健的统计推断")
            print(f"   LRP预后因子: 基于LRP值是否显著偏离0来判定")
            print(f"   - Driver(增加风险): LRP > 0")
            print(f"   - Protector(降低风险): LRP < 0")
    ...
    if args.feature_importance_only:
        print(f"\n✅ Bootstrap LRP特征重要性保存完成！处理了{len(all_cancer_summaries)}种癌症类型")
    else:
        print(f"\n✅ Bootstrap LRP分析完成！处理了{len(all_cancer_summaries)}种癌症类型")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="对Bootstrap模型进行LRP可解释性分析和预后因子识别")
    ...
    parser.add_argument('--lrp_dir', type=str, required=True, help='LRP分析结果保存目录')
