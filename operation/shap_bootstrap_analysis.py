...
def compute_shap_and_prognostic_factors(bootstrap_seed, args, num_features, feature_importance_only=False):
    """计算单个bootstrap模型的SHAP值并识别预后因子
    ...
    try:
        ...
        # 使用GradientExplainer (新式API，不需要tensorflow)
        explainer = shap.GradientExplainer(wrapped_model, background_t)
        ...
        print(f"GradientExplainer计算失败: {e}")
        return None
    ...
    # 创建输出目录
    # 输出路径: args.shap_dir/[癌症]/shap_feature_importance/
    importance_dir = os.path.join(args.shap_dir, args.cancer, 'shap_feature_importance')
    if not os.path.exists(importance_dir):
        os.makedirs(importance_dir, exist_ok=True)
        print(f"📁 创建目录: {importance_dir}")
    
    # 文件名: seed{X}_shap_ranking.csv
    importance_file = os.path.join(importance_dir, f"seed{bootstrap_seed}_shap_ranking.csv")
    feature_importance_df.to_csv(importance_file, index=False)
    print(f"✅ SHAP特征重要性排序已保存: {importance_file}")
    ...
    # 正确逻辑：直接基于SHAP值是否显著偏离0
    print(f"使用正确逻辑：基于SHAP值的直接生物学意义")
    print(f"Driver: SHAP均值显著 > 0 (增加风险)")
    print(f"Protector: SHAP均值显著 < 0 (降低风险)")
    
    for i in top_100_indices:
        shap_feature = shap_values[:, i]  # 该特征在所有OOB样本中的SHAP值
    ...
    result = {
        ...
        # SHAP分析结果 - FDR校正后
        'shap_prognostic_factors_fdr': prognostic_count_fdr,
        'shap_driver_factors_fdr': driver_count,
        'shap_protector_factors_fdr': protector_count,
        
        # SHAP分析结果 - 原始p值
        'shap_prognostic_factors_raw': prognostic_count_raw,
        'shap_driver_factors_raw': driver_count_raw,
        'shap_protector_factors_raw': protector_count_raw,
        ...
        'analysis_method': 'Bootstrap_SHAP_plus_Cox_Analysis'
    }
...
def analyze_bootstrap_cancer(args, num_features, num_bootstraps=100, feature_importance_only=False):
    ...
    for bootstrap_seed in range(1, num_bootstraps + 1):
        result = compute_shap_and_prognostic_factors(bootstrap_seed, args, num_features, feature_importance_only)
    ...
    # SHAP分析统计数据 - FDR校正后
    shap_prognostic_counts_fdr = [r['shap_prognostic_factors_fdr'] for r in all_results]
    shap_driver_counts_fdr = [r['shap_driver_factors_fdr'] for r in all_results]
    shap_protector_counts_fdr = [r['shap_protector_factors_fdr'] for r in all_results]
    
    # SHAP分析统计数据 - 原始p值
    shap_prognostic_counts_raw = [r['shap_prognostic_factors_raw'] for r in all_results]
    shap_driver_counts_raw = [r['shap_driver_factors_raw'] for r in all_results]
    shap_protector_counts_raw = [r['shap_protector_factors_raw'] for r in all_results]
    ...
    stats_summary = {
        ...
        # SHAP分析统计 - FDR校正后
        'shap_prognostic_factors_fdr_mean': np.mean(shap_prognostic_counts_fdr),
        'shap_prognostic_factors_fdr_std': np.std(shap_prognostic_counts_fdr),
        'shap_driver_factors_fdr_mean': np.mean(shap_driver_counts_fdr),
        'shap_driver_factors_fdr_std': np.std(shap_driver_counts_fdr),
        'shap_protector_factors_fdr_mean': np.mean(shap_protector_counts_fdr),
        'shap_protector_factors_fdr_std': np.std(shap_protector_counts_fdr),
        
        # SHAP分析统计 - 原始p值
        'shap_prognostic_factors_raw_mean': np.mean(shap_prognostic_counts_raw),
        'shap_prognostic_factors_raw_std': np.std(shap_prognostic_counts_raw),
        'shap_driver_factors_raw_mean': np.mean(shap_driver_counts_raw),
        'shap_driver_factors_raw_std': np.std(shap_driver_counts_raw),
        'shap_protector_factors_raw_mean': np.mean(shap_protector_counts_raw),
        'shap_protector_factors_raw_std': np.std(shap_protector_counts_raw),
    ...
    # 相关性分析
    if len(cindices) > 2:
        # C-index与SHAP预后因子总数的相关性 (原始p值 - 主要分析)
        corr_shap_prog_raw, p_shap_prog_raw = pearsonr(cindices, shap_prognostic_counts_raw)
        spearman_shap_prog_raw, sp_shap_prog_raw = spearmanr(cindices, shap_prognostic_counts_raw)
        
        # C-index与driver因子数量的相关性 (原始p值)
        corr_driver_raw, p_driver_raw = pearsonr(cindices, shap_driver_counts_raw)
        spearman_driver_raw, sp_driver_raw = spearmanr(cindices, shap_driver_counts_raw)
        
        # C-index与protector因子数量的相关性 (原始p值)
        corr_protector_raw, p_protector_raw = pearsonr(cindices, shap_protector_counts_raw)
        spearman_protector_raw, sp_protector_raw = spearmanr(cindices, shap_protector_counts_raw)
        ...
        # 附加：FDR校正后的相关性（用于对比）
        corr_shap_prog_fdr, p_shap_prog_fdr = pearsonr(cindices, shap_prognostic_counts_fdr)
        corr_cox_prog_fdr, p_cox_prog_fdr = pearsonr(cindices, cox_prognostic_counts_fdr)
        
        stats_summary.update({
            # SHAP相关性 - 原始p值（主要分析）
            'corr_cindex_shap_prognostic_raw_pearson': corr_shap_prog_raw,
            'corr_cindex_shap_prognostic_raw_pearson_p': p_shap_prog_raw,
            'corr_cindex_shap_prognostic_raw_spearman': spearman_shap_prog_raw,
            'corr_cindex_shap_prognostic_raw_spearman_p': sp_shap_prog_raw,
            ...
            # FDR校正后的相关性（辅助分析）
            'corr_cindex_shap_prognostic_fdr_pearson': corr_shap_prog_fdr,
            'corr_cindex_shap_prognostic_fdr_pearson_p': p_shap_prog_fdr,
            'corr_cindex_cox_prognostic_fdr_pearson': corr_cox_prog_fdr,
            'corr_cindex_cox_prognostic_fdr_pearson_p': p_cox_prog_fdr
        })
        
        print(f"=== SHAP相关性分析（使用原始p值预后因子数量）===")
        print(f"C-index vs SHAP预后因子总数 - Pearson: r={corr_shap_prog_raw:.3f}, p={p_shap_prog_raw:.3f}")
        ...
        print(f"对比FDR校正后 - SHAP: r={corr_shap_prog_fdr:.3f}, Cox: r={corr_cox_prog_fdr:.3f}")
    
    # 打印统计摘要
    print(f"\n=== {args.cancer} Bootstrap SHAP 统计摘要 ===")
    print(f"C-index: {stats_summary['cindex_mean']:.4f} ± {stats_summary['cindex_std']:.4f}")
    print(f"FDR校正 - SHAP预后因子: {stats_summary['shap_prognostic_factors_fdr_mean']:.1f} ± {stats_summary['shap_prognostic_factors_fdr_std']:.1f}")
    print(f"FDR校正 - Cox预后因子: {stats_summary['cox_prognostic_factors_fdr_mean']:.1f} ± {stats_summary['cox_prognostic_factors_fdr_std']:.1f}")
    
    # 创建结果保存目录
    results_dir = os.path.join(args.shap_dir, args.cancer)
    ...
    detailed_results_df = pd.DataFrame([{
        ...
        'shap_prognostic_factors_fdr': r['shap_prognostic_factors_fdr'],
        'shap_driver_factors_fdr': r['shap_driver_factors_fdr'],
        'shap_protector_factors_fdr': r['shap_protector_factors_fdr'],
        'shap_prognostic_factors_raw': r['shap_prognostic_factors_raw'],
        'shap_driver_factors_raw': r['shap_driver_factors_raw'],
        'shap_protector_factors_raw': r['shap_protector_factors_raw'],
    ...
    # 保存每个bootstrap的Cox分析详细结果（包含每个基因的SHAP统计信息）
    all_cox_details = []
    for r in all_results:
...
def create_bootstrap_correlation_plots(cancer, df, results_dir):
    """创建bootstrap相关性图（包含SHAP和Cox分析）- 🔧 添加拟合直线和统计信息"""
    ...
    # === 第一行：SHAP分析结果（使用原始p值预后因子数量）===
    # C-index vs SHAP预后因子总数
    plot_with_fit(axes[0,0], df['cindex'], df['shap_prognostic_factors_raw'], 
                  'C-index', 'SHAP Prognostic Factors (Raw p)', 
                  f'{cancer}: C-index vs SHAP Prognostic Factors (Bootstrap)', 'purple')
    
    # C-index vs Driver因子数量
    plot_with_fit(axes[0,1], df['cindex'], df['shap_driver_factors_raw'], 
                  'C-index', 'SHAP Driver Factors (Raw p)', 
                  f'{cancer}: C-index vs SHAP Driver Factors (Bootstrap)', 'red')
    
    # C-index vs Protector因子数量
    plot_with_fit(axes[0,2], df['cindex'], df['shap_protector_factors_raw'], 
                  'C-index', 'SHAP Protector Factors (Raw p)', 
                  f'{cancer}: C-index vs SHAP Protector Factors (Bootstrap)', 'blue')
...
def main(args):
    ...
    os.makedirs(args.shap_dir, exist_ok=True)
    ...
    for cancer in cancer_list:
        ...
        num_features = 2000
        print(f"\n{'='*60}")
        print(f"正在处理癌症类型: {cancer} (Bootstrap SHAP)")
    ...
    if all_cancer_summaries:
        ...
        summary_file = os.path.join(args.shap_dir, "all_cancers_bootstrap_summary.csv")
        ...
        if not args.feature_importance_only:
            ...
            print(f"平均SHAP预后因子(FDR): {all_summaries_df['shap_prognostic_factors_fdr_mean'].mean():.1f}")
    ...
    if args.feature_importance_only:
        print(f"\n✅ Bootstrap SHAP特征重要性保存完成！处理了{len(all_cancer_summaries)}种癌症类型")
    else:
        print(f"\n✅ Bootstrap SHAP分析完成！处理了{len(all_cancer_summaries)}种癌症类型")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="对Bootstrap模型进行SHAP可解释性分析和预后因子识别")
    ...
    parser.add_argument('--shap_dir', type=str, required=True, help='SHAP分析结果保存目录')
