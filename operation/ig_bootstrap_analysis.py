...
def perform_cox_analysis(X_data, data_df, top_100_indices, feature_names):
    ...
    参数:
    - X_data: 特征数据（bootstrap全数据集）
    - data_df: 数据框（bootstrap全数据集）
    ...
    if 'survival_months' in data_df.columns and 'censorship' in data_df.columns:
        ...
        print(f"可用列名: {list(data_df.columns)}")
        return [], 0, 0, 0, 0
...
def compute_ig_and_prognostic_factors(bootstrap_seed, args, num_features, feature_importance_only=False):
    """计算单个bootstrap模型的IG值并识别预后因子
    ...
    print(f"计算IG值...")
    # 创建包装模型
    wrapped_model = RiskModelWrapper(model)
    ...
    attributions = ig.attribute(X_oob_t, baselines=baseline, n_steps=50)
    attributions_np = attributions.detach().cpu().numpy()
    
    # 计算特征重要性（用于筛选Top 100）
    importance = np.abs(attributions_np).mean(axis=0)
    ...
    # 创建输出目录
    # 输出路径: args.ig_dir/[癌症]/ig_feature_importance/
    importance_dir = os.path.join(args.ig_dir, args.cancer, 'ig_feature_importance')
    if not os.path.exists(importance_dir):
        os.makedirs(importance_dir, exist_ok=True)
        print(f"📁 创建目录: {importance_dir}")
    
    # 文件名: seed{X}_ig_ranking.csv
    importance_file = os.path.join(importance_dir, f"seed{bootstrap_seed}_ig_ranking.csv")
    feature_importance_df.to_csv(importance_file, index=False)
    print(f"✅ IG特征重要性排序已保存: {importance_file}")
    ...
    # 正确逻辑：直接基于IG值是否显著偏离0
    print(f"使用正确逻辑：基于IG值的直接生物学意义")
    print(f"Driver: IG均值显著 > 0 (增加风险)")
    print(f"Protector: IG均值显著 < 0 (降低风险)")
    
    for i in top_100_indices:
        ig_feature = attributions_np[:, i]  # 该特征在所有OOB样本中的IG值
        
        # 核心逻辑：测试IG值是否显著偏离0
        try:
            stat, pval = ttest_1samp(ig_feature, 0)
        except:
            pval = 1.0
            stat = 0
        
        mean_ig = np.mean(ig_feature)
        std_ig = np.std(ig_feature)
        
        # 计算一致性指标
        consistency_ratio = np.abs(np.sign(ig_feature).sum()) / len(ig_feature)
        positive_ratio = (ig_feature > 0).mean()
        
        prognostic_factors.append({
            'gene': feature_names[i],
            'importance': importance[i],
            'mean_ig': mean_ig,
            'abs_mean_ig': abs(mean_ig),  # 绝对平均值
            'std_ig': std_ig,
            'ttest_stat': stat,
            'p_value': pval,
    ...
    if p_values:
        ...
        for i, (factor, adj_p, sig) in enumerate(zip(prognostic_factors, p_adj, reject)):
            ...
            if sig:  # FDR显著偏离0
                significant_count += 1
                # 正确且简单的逻辑：直接基于IG均值的符号
                if factor['mean_ig'] > 0:
                    factor['type'] = 'driver'      # IG > 0: 增加风险
                    driver_count += 1
                elif factor['mean_ig'] < 0:
                    factor['type'] = 'protector'   # IG < 0: 降低风险
                    protector_count += 1
                else:
    ...
    result = {
        ...
        # IG分析结果 - FDR校正后
        'ig_prognostic_factors_fdr': prognostic_count_fdr,
        'ig_driver_factors_fdr': driver_count,
        'ig_protector_factors_fdr': protector_count,
        
        # IG分析结果 - 原始p值
        'ig_prognostic_factors_raw': prognostic_count_raw,
        'ig_driver_factors_raw': driver_count_raw,
        'ig_protector_factors_raw': protector_count_raw,
        
        'prognostic_factors': prognostic_factors,  # 每个基因的详细IG统计信息在这里
        ...
        'analysis_method': 'Bootstrap_IG_plus_Cox_Analysis'
    }
    ...
    print(f"=== IG分析结果 ===")
    print(f"统计显著因子数量: {significant_count} (IG值显著偏离0)")
...
def analyze_bootstrap_cancer(args, num_features, num_bootstraps=100, feature_importance_only=False):
    ...
    for bootstrap_seed in range(1, num_bootstraps + 1):
        result = compute_ig_and_prognostic_factors(bootstrap_seed, args, num_features, feature_importance_only)
    ...
    # IG分析统计数据 - FDR校正后
    ig_prognostic_counts_fdr = [r['ig_prognostic_factors_fdr'] for r in all_results]
    ig_driver_counts_fdr = [r['ig_driver_factors_fdr'] for r in all_results]
    ig_protector_counts_fdr = [r['ig_protector_factors_fdr'] for r in all_results]
    
    # IG分析统计数据 - 原始p值
    ig_prognostic_counts_raw = [r['ig_prognostic_factors_raw'] for r in all_results]
    ig_driver_counts_raw = [r['ig_driver_factors_raw'] for r in all_results]
    ig_protector_counts_raw = [r['ig_protector_factors_raw'] for r in all_results]
    ...
    stats_summary = {
        ...
        # IG分析统计 - FDR校正后
        'ig_prognostic_factors_fdr_mean': np.mean(ig_prognostic_counts_fdr),
        'ig_prognostic_factors_fdr_std': np.std(ig_prognostic_counts_fdr),
        'ig_driver_factors_fdr_mean': np.mean(ig_driver_counts_fdr),
        'ig_driver_factors_fdr_std': np.std(ig_driver_counts_fdr),
        'ig_protector_factors_fdr_mean': np.mean(ig_protector_counts_fdr),
        'ig_protector_factors_fdr_std': np.std(ig_protector_counts_fdr),
        
        # IG分析统计 - 原始p值
        'ig_prognostic_factors_raw_mean': np.mean(ig_prognostic_counts_raw),
        'ig_prognostic_factors_raw_std': np.std(ig_prognostic_counts_raw),
        'ig_driver_factors_raw_mean': np.mean(ig_driver_counts_raw),
        'ig_driver_factors_raw_std': np.std(ig_driver_counts_raw),
        'ig_protector_factors_raw_mean': np.mean(ig_protector_counts_raw),
        'ig_protector_factors_raw_std': np.std(ig_protector_counts_raw),
    ...
    if len(cindices) > 2:
        # C-index与IG预后因子总数的相关性 (原始p值 - 主要分析)
        corr_ig_prog_raw, p_ig_prog_raw = pearsonr(cindices, ig_prognostic_counts_raw)
        spearman_ig_prog_raw, sp_ig_prog_raw = spearmanr(cindices, ig_prognostic_counts_raw)
        
        # C-index与driver因子数量的相关性 (原始p值)
        corr_driver_raw, p_driver_raw = pearsonr(cindices, ig_driver_counts_raw)
        spearman_driver_raw, sp_driver_raw = spearmanr(cindices, ig_driver_counts_raw)
        
        # C-index与protector因子数量的相关性 (原始p值)
        corr_protector_raw, p_protector_raw = pearsonr(cindices, ig_protector_counts_raw)
        spearman_protector_raw, sp_protector_raw = spearmanr(cindices, ig_protector_counts_raw)
        ...
        # 附加：FDR校正后的相关性（用于对比）
        corr_ig_prog_fdr, p_ig_prog_fdr = pearsonr(cindices, ig_prognostic_counts_fdr)
        corr_cox_prog_fdr, p_cox_prog_fdr = pearsonr(cindices, cox_prognostic_counts_fdr)
        
        stats_summary.update({
            # IG相关性 - 原始p值（主要分析）
            'corr_cindex_ig_prognostic_raw_pearson': corr_ig_prog_raw,
            'corr_cindex_ig_prognostic_raw_pearson_p': p_ig_prog_raw,
            'corr_cindex_ig_prognostic_raw_spearman': spearman_ig_prog_raw,
            'corr_cindex_ig_prognostic_raw_spearman_p': sp_ig_prog_raw,
            ...
            # FDR校正后的相关性（辅助分析）
            'corr_cindex_ig_prognostic_fdr_pearson': corr_ig_prog_fdr,
            'corr_cindex_ig_prognostic_fdr_pearson_p': p_ig_prog_fdr,
            'corr_cindex_cox_prognostic_fdr_pearson': corr_cox_prog_fdr,
            'corr_cindex_cox_prognostic_fdr_pearson_p': p_cox_prog_fdr
        })
        
        print(f"=== IG相关性分析（使用原始p值预后因子数量）===")
        print(f"C-index vs IG预后因子总数 - Pearson: r={corr_ig_prog_raw:.3f}, p={p_ig_prog_raw:.3f}")
        ...
        print(f"对比FDR校正后 - IG: r={corr_ig_prog_fdr:.3f}, Cox: r={corr_cox_prog_fdr:.3f}")
    
    # 保存详细结果
    results_dir = os.path.join(args.ig_dir, args.cancer)
    ...
    detailed_results_df = pd.DataFrame([{
        ...
        # IG分析结果 - FDR校正后
        'ig_prognostic_factors_fdr': r['ig_prognostic_factors_fdr'],
        'ig_driver_factors_fdr': r['ig_driver_factors_fdr'],
        'ig_protector_factors_fdr': r['ig_protector_factors_fdr'],
        
        # IG分析结果 - 原始p值
        'ig_prognostic_factors_raw': r['ig_prognostic_factors_raw'],
        'ig_driver_factors_raw': r['ig_driver_factors_raw'],
        'ig_protector_factors_raw': r['ig_protector_factors_raw'],
    ...
    # 保存每个bootstrap的Cox分析详细结果（包含每个基因的IG统计信息）
    all_cox_details = []
    for r in all_results:
        if 'cox_results' in r and r['cox_results'] and 'prognostic_factors' in r:
            # 创建基因名到IG统计信息的映射
            ig_stats_map = {pf['gene']: pf for pf in r['prognostic_factors']}
            ...
            for cox_result in r['cox_results']:
                ...
                # 添加该基因的IG统计信息
                if gene_name in ig_stats_map:
                    ig_stats = ig_stats_map[gene_name]
                    cox_detail.update({
                        'mean_ig': ig_stats['mean_ig'],
                        'abs_mean_ig': ig_stats['abs_mean_ig'],
                        'ig_importance': ig_stats['importance'],
                        'ig_p_value': ig_stats['p_value'],
                        'ig_p_adj': ig_stats.get('p_adj', None),
                        'ig_type': ig_stats.get('type', 'not_significant')
                    })
...
def create_bootstrap_correlation_plots(cancer, df, results_dir):
    """创建bootstrap相关性图（包含IG和Cox分析）- 🔧 添加拟合直线和统计信息"""
    ...
    # === 第一行：IG分析结果（使用原始p值预后因子数量）===
    # C-index vs IG预后因子总数
    plot_with_fit(axes[0,0], df['cindex'], df['ig_prognostic_factors_raw'], 
                  'C-index', 'IG Prognostic Factors (Raw p)', 
                  f'{cancer}: C-index vs IG Prognostic Factors (Bootstrap)', 'purple')
    
    # C-index vs Driver因子数量
    plot_with_fit(axes[0,1], df['cindex'], df['ig_driver_factors_raw'], 
                  'C-index', 'IG Driver Factors (Raw p)', 
                  f'{cancer}: C-index vs IG Driver Factors (Bootstrap)', 'red')
    
    # C-index vs Protector因子数量
    plot_with_fit(axes[0,2], df['cindex'], df['ig_protector_factors_raw'], 
                  'C-index', 'IG Protector Factors (Raw p)', 
                  f'{cancer}: C-index vs IG Protector Factors (Bootstrap)', 'blue')
...
def main(args):
    ...
    os.makedirs(args.ig_dir, exist_ok=True)
    ...
    for cancer in cancer_list:
        ...
        num_features = 2000
        print(f"\n{'='*60}")
        print(f"正在处理癌症类型: {cancer} (Bootstrap IG)")
    ...
    if all_cancer_summaries:
        ...
        summary_df.to_csv(os.path.join(args.ig_dir, "all_cancers_bootstrap_summary.csv"), index=False)
        print(f"\n✅ 所有癌症汇总结果已保存到: {os.path.join(args.ig_dir, 'all_cancers_bootstrap_summary.csv')}")
        
        # 打印总体统计
        if not args.feature_importance_only:
            print(f"\n{'='*60}")
            print("所有癌症类型Bootstrap汇总 (IG + Cox分析):")
            print(f"{'='*60}")
            
            # IG分析汇总 - 主要结果（原始p值）
            print("=== IG分析汇总（主要结果：使用原始p值预后因子数量）===")
            ig_cols_raw = ['cancer', 'cindex_mean', 'ig_prognostic_factors_raw_mean', 
                          'corr_cindex_ig_prognostic_raw_pearson', 'corr_cindex_ig_prognostic_raw_pearson_p']
            print(summary_df[ig_cols_raw].to_string(index=False))
            ...
            # 对比：FDR校正后的结果
            print("\n=== 对比：FDR校正后的相关性 ===")
            compare_cols = ['cancer', 'ig_prognostic_factors_fdr_mean', 'cox_prognostic_factors_fdr_mean',
                           'corr_cindex_ig_prognostic_fdr_pearson', 'corr_cindex_cox_prognostic_fdr_pearson']
            print(summary_df[compare_cols].to_string(index=False))
            
            print(f"\n💡 说明:")
            print(f"   🎯 主要分析：使用原始p值（校正前）的预后因子数量计算相关性")
            print(f"   📊 可视化：散点图包含拟合直线、相关系数和显著性检验")
            print(f"   🔄 Bootstrap: 对100个重抽样模型进行IG分析，提供更稳健的统计推断")
            print(f"   IG预后因子: 基于IG值是否显著偏离0来判定")
            print(f"   - Driver(增加风险): IG > 0")
            print(f"   - Protector(降低风险): IG < 0")
    ...
    if args.feature_importance_only:
        print(f"\n✅ Bootstrap IG特征重要性保存完成！处理了{len(all_cancer_summaries)}种癌症类型")
    else:
        print(f"\n✅ Bootstrap IG分析完成！处理了{len(all_cancer_summaries)}种癌症类型")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="对Bootstrap模型进行IG可解释性分析和预后因子识别")
    ...
    parser.add_argument('--ig_dir', type=str, required=True, help='IG分析结果保存目录')
