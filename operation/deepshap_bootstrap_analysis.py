...
def compute_deepshap_and_prognostic_factors(bootstrap_seed, args, num_features, feature_importance_only=False):
    """计算单个bootstrap模型的DeepSHAP值并识别预后因子
    ...
    print(f"计算DeepSHAP值...")
    # 创建DeepExplainer (适用于深度学习模型)
    # 由于样本数量较大，我们使用训练集的随机子集作为background
    background_size = min(100, len(X_train))  # 限制background大小
    ...
    try:
        ...
        shap_values = explainer.shap_values(X_oob_t, check_additivity=False)
        ...
        if isinstance(shap_values, list):
            shap_values = shap_values[0]  # 如果是list，取第一个元素
        
        if shap_values.ndim == 3 and shap_values.shape[2] == 1:
            shap_values = shap_values[:, :, 0]
    except Exception as e:
        print(f"DeepExplainer计算失败: {e}")
        print(f"尝试备用方法...")
        try:
            # 备用方法：逐个样本计算
            shap_values_list = []
            for i in range(len(X_oob_t)):
                ...
                sv = explainer.shap_values(single_sample, check_additivity=False)
    ...
    # 创建输出目录
    # 输出路径: args.deepshap_dir/[癌症]/deepshap_feature_importance/
    importance_dir = os.path.join(args.deepshap_dir, args.cancer, 'deepshap_feature_importance')
    if not os.path.exists(importance_dir):
        os.makedirs(importance_dir, exist_ok=True)
        print(f"📁 创建目录: {importance_dir}")

    # 文件名: seed{X}_deepshap_ranking.csv
    importance_file = os.path.join(importance_dir, f"seed{bootstrap_seed}_deepshap_ranking.csv")
    feature_importance_df.to_csv(importance_file, index=False)
    print(f"✅ DeepSHAP特征重要性排序已保存: {importance_file}")
    ...
    # 正确逻辑：直接基于DeepSHAP值是否显著偏离0
    print(f"使用正确逻辑：基于DeepSHAP值的直接生物学意义")
    print(f"Driver: DeepSHAP均值显著 > 0 (增加风险)")
    print(f"Protector: DeepSHAP均值显著 < 0 (降低风险)")

    for i in top_100_indices:
        deepshap_feature = shap_values[:, i]  # 该特征在所有OOB样本中的DeepSHAP值
        
        # 核心逻辑：测试DeepSHAP值是否显著偏离0
        try:
            stat, pval = ttest_1samp(deepshap_feature, 0)
        except:
            pval = 1.0
            stat = 0
        
        mean_deepshap = np.mean(deepshap_feature)
        std_deepshap = np.std(deepshap_feature)
        
        # 计算一致性指标
        consistency_ratio = np.abs(np.sign(deepshap_feature).sum()) / len(deepshap_feature)
        positive_ratio = (deepshap_feature > 0).mean()
        
        prognostic_factors.append({
            'gene': feature_names[i],
            'importance': importance[i],
            'mean_deepshap': mean_deepshap,
            'abs_mean_deepshap': abs(mean_deepshap),  # 绝对平均值
            'std_deepshap': std_deepshap,
            'ttest_stat': stat,
            'p_value': pval,
    ...
    if p_values:
        ...
        for i, (factor, adj_p, sig) in enumerate(zip(prognostic_factors, p_adj, reject)):
            ...
            if sig:  # FDR显著偏离0
                significant_count += 1
                # 正确且简单的逻辑：直接基于DeepSHAP均值的符号
                if factor['mean_deepshap'] > 0:
                    factor['type'] = 'driver'      # DeepSHAP > 0: 增加风险
                    driver_count += 1
                elif factor['mean_deepshap'] < 0:
                    factor['type'] = 'protector'   # DeepSHAP < 0: 降低风险
                    protector_count += 1
                else:
    ...
    result = {
        ...
        # DeepSHAP分析结果 - FDR校正后
        'deepshap_prognostic_factors_fdr': prognostic_count_fdr,
        'deepshap_driver_factors_fdr': driver_count,
        'deepshap_protector_factors_fdr': protector_count,
        
        # DeepSHAP分析结果 - 原始p值
        'deepshap_prognostic_factors_raw': prognostic_count_raw,
        'deepshap_driver_factors_raw': driver_count_raw,
        'deepshap_protector_factors_raw': protector_count_raw,
        
        'prognostic_factors': prognostic_factors,  # 每个基因的详细DeepSHAP统计信息在这里
        ...
        'analysis_method': 'Bootstrap_DeepSHAP_plus_Cox_Analysis'
    }

    print(f"C-index: {cindex:.4f}")
    print(f"=== DeepSHAP分析结果 ===")
    print(f"统计显著因子数量: {significant_count} (DeepSHAP值显著偏离0)")
...
def analyze_bootstrap_cancer(args, num_features, num_bootstraps=100, feature_importance_only=False):
    ...
    for bootstrap_seed in range(1, num_bootstraps + 1):
        result = compute_deepshap_and_prognostic_factors(bootstrap_seed, args, num_features, feature_importance_only)
    ...
    # DeepSHAP分析统计数据 - FDR校正后
    deepshap_prognostic_counts_fdr = [r['deepshap_prognostic_factors_fdr'] for r in all_results]
    deepshap_driver_counts_fdr = [r['deepshap_driver_factors_fdr'] for r in all_results]
    deepshap_protector_counts_fdr = [r['deepshap_protector_factors_fdr'] for r in all_results]

    # DeepSHAP分析统计数据 - 原始p值
    deepshap_prognostic_counts_raw = [r['deepshap_prognostic_factors_raw'] for r in all_results]
    deepshap_driver_counts_raw = [r['deepshap_driver_factors_raw'] for r in all_results]
    deepshap_protector_counts_raw = [r['deepshap_protector_factors_raw'] for r in all_results]
    ...
    stats_summary = {
        ...
        # DeepSHAP分析统计 - FDR校正后
        'deepshap_prognostic_factors_fdr_mean': np.mean(deepshap_prognostic_counts_fdr),
        'deepshap_prognostic_factors_fdr_std': np.std(deepshap_prognostic_counts_fdr),
        'deepshap_driver_factors_fdr_mean': np.mean(deepshap_driver_counts_fdr),
        'deepshap_driver_factors_fdr_std': np.std(deepshap_driver_counts_fdr),
        'deepshap_protector_factors_fdr_mean': np.mean(deepshap_protector_counts_fdr),
        'deepshap_protector_factors_fdr_std': np.std(deepshap_protector_counts_fdr),
        
        # DeepSHAP分析统计 - 原始p值
        'deepshap_prognostic_factors_raw_mean': np.mean(deepshap_prognostic_counts_raw),
        'deepshap_prognostic_factors_raw_std': np.std(deepshap_prognostic_counts_raw),
        'deepshap_driver_factors_raw_mean': np.mean(deepshap_driver_counts_raw),
        'deepshap_driver_factors_raw_std': np.std(deepshap_driver_counts_raw),
        'deepshap_protector_factors_raw_mean': np.mean(deepshap_protector_counts_raw),
        'deepshap_protector_factors_raw_std': np.std(deepshap_protector_counts_raw),
    ...
    # 相关性分析
    if len(cindices) > 2:
        # C-index与DeepSHAP预后因子总数的相关性 (原始p值 - 主要分析)
        corr_deepshap_prog_raw, p_deepshap_prog_raw = pearsonr(cindices, deepshap_prognostic_counts_raw)
        spearman_deepshap_prog_raw, sp_deepshap_prog_raw = spearmanr(cindices, deepshap_prognostic_counts_raw)
        
        # C-index与driver因子数量的相关性 (原始p值)
        corr_driver_raw, p_driver_raw = pearsonr(cindices, deepshap_driver_counts_raw)
        spearman_driver_raw, sp_driver_raw = spearmanr(cindices, deepshap_driver_counts_raw)
        
        # C-index与protector因子数量的相关性 (原始p值)
        corr_protector_raw, p_protector_raw = pearsonr(cindices, deepshap_protector_counts_raw)
        spearman_protector_raw, sp_protector_raw = spearmanr(cindices, deepshap_protector_counts_raw)
        ...
        # 附加：FDR校正后的相关性（用于对比）
        corr_deepshap_prog_fdr, p_deepshap_prog_fdr = pearsonr(cindices, deepshap_prognostic_counts_fdr)
        corr_cox_prog_fdr, p_cox_prog_fdr = pearsonr(cindices, cox_prognostic_counts_fdr)
        
        stats_summary.update({
            # DeepSHAP相关性 - 原始p值（主要分析）
            'corr_cindex_deepshap_prognostic_raw_pearson': corr_deepshap_prog_raw,
            'corr_cindex_deepshap_prognostic_raw_pearson_p': p_deepshap_prog_raw,
            'corr_cindex_deepshap_prognostic_raw_spearman': spearman_deepshap_prog_raw,
            'corr_cindex_deepshap_prognostic_raw_spearman_p': sp_deepshap_prog_raw,
            ...
            # FDR校正后的相关性（辅助分析）
            'corr_cindex_deepshap_prognostic_fdr_pearson': corr_deepshap_prog_fdr,
            'corr_cindex_deepshap_prognostic_fdr_pearson_p': p_deepshap_prog_fdr,
            'corr_cindex_cox_prognostic_fdr_pearson': corr_cox_prog_fdr,
            'corr_cindex_cox_prognostic_fdr_pearson_p': p_cox_prog_fdr
        })
        
        print(f"=== DeepSHAP相关性分析（使用原始p值预后因子数量）===")
        print(f"C-index vs DeepSHAP预后因子总数 - Pearson: r={corr_deepshap_prog_raw:.3f}, p={p_deepshap_prog_raw:.3f}")
        ...
        print(f"对比FDR校正后 - DeepSHAP: r={corr_deepshap_prog_fdr:.3f}, Cox: r={corr_cox_prog_fdr:.3f}")

    # 打印统计摘要
    print(f"\n=== {args.cancer} Bootstrap DeepSHAP 统计摘要 ===")
    print(f"C-index: {stats_summary['cindex_mean']:.4f} ± {stats_summary['cindex_std']:.4f}")
    print(f"FDR校正 - DeepSHAP预后因子: {stats_summary['deepshap_prognostic_factors_fdr_mean']:.1f} ± {stats_summary['deepshap_prognostic_factors_fdr_std']:.1f}")
    print(f"FDR校正 - Cox预后因子: {stats_summary['cox_prognostic_factors_fdr_mean']:.1f} ± {stats_summary['cox_prognostic_factors_fdr_std']:.1f}")

    # 创建结果保存目录
    results_dir = os.path.join(args.deepshap_dir, args.cancer)
    ...
    detailed_results_df = pd.DataFrame([{
        ...
        'deepshap_prognostic_factors_fdr': r['deepshap_prognostic_factors_fdr'],
        'deepshap_driver_factors_fdr': r['deepshap_driver_factors_fdr'],
        'deepshap_protector_factors_fdr': r['deepshap_protector_factors_fdr'],
        'deepshap_prognostic_factors_raw': r['deepshap_prognostic_factors_raw'],
        'deepshap_driver_factors_raw': r['deepshap_driver_factors_raw'],
        'deepshap_protector_factors_raw': r['deepshap_protector_factors_raw'],
    ...
    # 保存每个bootstrap的Cox分析详细结果（包含每个基因的DeepSHAP统计信息）
    all_cox_details = []
    for r in all_results:
        if 'cox_results' in r and r['cox_results'] and 'prognostic_factors' in r:
            # 创建基因名到DeepSHAP统计信息的映射
            deepshap_stats_map = {pf['gene']: pf for pf in r['prognostic_factors']}
            ...
            for cox_result in r['cox_results']:
                ...
                # 添加该基因的DeepSHAP统计信息
                if gene_name in deepshap_stats_map:
                    deepshap_stats = deepshap_stats_map[gene_name]
                    cox_detail.update({
                        'mean_deepshap': deepshap_stats['mean_deepshap'],
                        'abs_mean_deepshap': deepshap_stats['abs_mean_deepshap'],
                        'deepshap_importance': deepshap_stats['importance'],
                        'deepshap_p_value': deepshap_stats['p_value'],
                        'deepshap_p_adj': deepshap_stats.get('p_adj', None),
                        'deepshap_type': deepshap_stats.get('type', 'not_significant')
                    })
...
def create_bootstrap_correlation_plots(cancer, df, results_dir):
    """创建bootstrap相关性图（包含DeepSHAP和Cox分析）- 🔧 添加拟合直线和统计信息"""
    ...
    # === 第一行：DeepSHAP分析结果（使用原始p值预后因子数量）===
    # C-index vs DeepSHAP预后因子总数
    plot_with_fit(axes[0,0], df['cindex'], df['deepshap_prognostic_factors_raw'], 
                'C-index', 'DeepSHAP Prognostic Factors (Raw p)', 
                f'{cancer}: C-index vs DeepSHAP Prognostic Factors (Bootstrap)', 'purple')

    # C-index vs Driver因子数量
    plot_with_fit(axes[0,1], df['cindex'], df['deepshap_driver_factors_raw'], 
                'C-index', 'DeepSHAP Driver Factors (Raw p)', 
                f'{cancer}: C-index vs DeepSHAP Driver Factors (Bootstrap)', 'red')

    # C-index vs Protector因子数量
    plot_with_fit(axes[0,2], df['cindex'], df['deepshap_protector_factors_raw'], 
                'C-index', 'DeepSHAP Protector Factors (Raw p)', 
                f'{cancer}: C-index vs DeepSHAP Protector Factors (Bootstrap)', 'blue')
...
def main(args):
    ...
    os.makedirs(args.deepshap_dir, exist_ok=True)
    ...
    for cancer in cancer_list:
        ...
        num_features = 2000
        print(f"\n{'='*60}")
        print(f"正在处理癌症类型: {cancer} (Bootstrap DeepSHAP)")
    ...
    if all_cancer_summaries:
        ...
        summary_file = os.path.join(args.deepshap_dir, "all_cancers_bootstrap_summary.csv")
        ...
        if not args.feature_importance_only:
            ...
            print(f"平均DeepSHAP预后因子(FDR): {all_summaries_df['deepshap_prognostic_factors_fdr_mean'].mean():.1f}")
    ...
    if args.feature_importance_only:
        print(f"\n✅ Bootstrap DeepSHAP特征重要性保存完成！处理了{len(all_cancer_summaries)}种癌症类型")
    else:
        print(f"\n✅ Bootstrap DeepSHAP分析完成！处理了{len(all_cancer_summaries)}种癌症类型")
    
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="对Bootstrap模型进行DeepSHAP可解释性分析和预后因子识别")
    ...
    parser.add_argument('--deepshap_dir', type=str, required=True, help='DeepSHAP分析结果保存目录')
