...
def compute_ig_and_prognostic_factors(repeat, fold, args, num_features):
    """计算单个实验的IG值并识别预后因子
    
    使用正确逻辑：
    - Driver因子: IG均值显著 > 0 (对风险的正向贡献)
    - Protector因子: IG均值显著 < 0 (对风险的负向贡献)
    - 统计检验: t检验测试IG值是否显著偏离0
    """
    ...
    baseline = X_train_tensor.mean(dim=0, keepdim=True).to(X_val_tensor.device)
    
    print(f"计算IG值...")
    # 计算风险分数
    with torch.no_grad():
        logits = model(x_omic=X_val_tensor)
        hazards = torch.sigmoid(logits)
        survival = torch.cumprod(1 - hazards, dim=1)
    ...
    IG_all = ig.attribute(X_val_tensor, baselines=baseline, n_steps=50).detach().cpu().numpy()
    ...
    importance_file = os.path.join(importance_dir, f"repeat{repeat}_fold{fold}_ig_feature_importance_ranking.csv")
    ...
    for i in top_100_indices:
        ig_feature = IG_all[:, i]  # 该特征在所有样本中的IG值
    ...
    # 不需要在实验级别汇总IG值，因为每个基因已经有自己的IG统计信息
    ...
    result = {
        ...
        'analysis_method': 'IG_plus_Cox_Analysis'  # 标记使用IG+Cox分析
    }
...
def analyze_cancer_type(args, num_features, num_repeats=10, num_folds=5):
    ...
    for repeat in range(num_repeats):
        for fold in range(num_folds):
            result = compute_ig_and_prognostic_factors(repeat, fold, args, num_features)
    ...
    # 保存每个实验的Cox分析详细结果（包含每个基因的IG统计信息）
    all_cox_details = []
    for r in all_results:
...
def main(args):
    """主函数 - IG分析 + Cox回归分析"""
    print("🔥 综合分析框架: IG + Cox回归")
    print("=== IG分析部分 ===")
    print("   ✅ Driver因子: IG均值显著 > 0 (增加风险)")
    print("   ✅ Protector因子: IG均值显著 < 0 (降低风险)")
    print("   ✅ 统计检验: t检验测试IG值是否显著偏离0")
    print("   ✅ 避免循环论证问题")
    print("=== Cox回归分析部分 ===")
    ...
    if all_cancer_summaries:
        ...
        summary_df.to_csv(os.path.join(args.ig_dir, "all_cancers_summary.csv"), index=False)
        
        print(f"\n{'='*60}")
        print("所有癌症类型汇总 (IG + Cox分析):")
        print(f"{'='*60}")
        ...
        print(f"   📁 特征重要性文件: {args.ig_dir}/[癌症类型]/feature_importance/")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="单独分析每个实验的IG值和预后因子")
